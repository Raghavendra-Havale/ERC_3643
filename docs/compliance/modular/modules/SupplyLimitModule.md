...
*getter for `supplyLimits` variable  returns supply limit*
...
this event is emitted when the supply limit has been set.  `_compliance` is the compliance address.  `_limit` is the max amount of tokens in circulation.
